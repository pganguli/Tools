"""
Replay a power-trace script on a programmable power supply to simulate
intermittent energy harvesting.

Reads a CSV trace file (step, period_ms, voltage_V, current_A) and commands
the power supply (BK9171B or Keithley 2280S) to step through the trace in
real time.  A background thread handles the timing loop; the main thread
optionally exposes a stop socket for the experiment harness.

Usage: python control-power-supply.py --power-trace TRACE.csv [--ip HOST]
"""
...
def main():
    ...
    normalized_power_trace = parse_power_script(
        args.script,
        args.voltage,
        args.normalized_average_current,
        args.normalized_max_current,
    )
